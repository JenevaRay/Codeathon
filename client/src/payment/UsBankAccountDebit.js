...
const UsBankAccountDebitForm = () => {
  ...
  return (
    <>
      ...
      {confirming && (
        <UsBankAccountDebitConfirmationForm
...
const UsBankAccountDebitConfirmationForm = ({ clientSecret, addMessage }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      addMessage(
        `<a href="${paymentIntent.next_action?.verify_with_microdeposits?.hosted_verification_url}">verify microdeposits</a>.`,
      );
    }
    addMessage(`Payment ${paymentIntent.status}: ${paymentIntent.id}`);
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      <p>
        By clicking Accept, you authorize <em>YOUR BUSINESS NAME</em> to debit
        the bank account specified above for any amount owed for charges arising
        from your use of <em>YOUR BUSINESS NAME</em>’ services and/or purchase
        of products from
        <em>YOUR BUSINESS NAME</em>, pursuant to <em>YOUR BUSINESS NAME</em>’
        website and terms, until this authorization is revoked. You may amend or
        cancel this authorization at any time by providing notice to{' '}
        <em>YOUR BUSINESS NAME</em> with 30 (thirty) days notice.
      </p>

      <p>
        If you use <em>YOUR BUSINESS NAME</em>’ services or purchase additional
        products periodically pursuant to <em>YOUR BUSINESS NAME</em>’ terms,
        you authorize <em>YOUR BUSINESS NAME</em> to debit your bank account
